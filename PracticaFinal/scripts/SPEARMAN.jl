...
# ===============================
# Spearman SelectKBest Filter
# ===============================
# Definir la estructura del modelo
mutable struct SpearmanFilter <: MLJModelInterface.Supervised
    k::Int
end

# Constructor con argumentos por nombre (estilo MLJ)
SpearmanFilter(; k::Int=2) = SpearmanFilter(k)
...
function fit(model::SpearmanFilter, verbosity::Int, X, y)
    ...
    # Calcular correlación de Spearman para cada característica
    ...
    for j in 1:n_features
        ...
        try
            # Calcular correlación de Spearman usando corspearman de StatsBase
            correlations[j] = abs(corspearman(feature, y_numeric))
        catch e
...
function transform(model::SpearmanFilter, fitresult, X)
...
input_scitype(::Type{<:SpearmanFilter}) = Table(Continuous)
target_scitype(::Type{<:SpearmanFilter}) = AbstractVector{<:Finite}
output_scitype(::Type{<:SpearmanFilter}) = Table(Continuous)